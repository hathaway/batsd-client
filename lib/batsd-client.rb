...
module Batsd
	class Client
	    ...
	    attr_accessor :host, :port, :remote, :timeout
	    ...
	    def initialize(options={})
	        ...
	        connect!
	        puts "Batsd: Connected to #{self.host} on port #{self.port}"
	    end
	    ...
	    def values(metric_name, start_timestamp, end_timestamp=Time.now, attempt=0)
	        ...
	        if values[metric_name].nil?
	            puts "Batsd: #{Thread.current} #{self.remote.addr} Values returned weren't the same as what was asked for (expecting #{metric_name}, got #{values.keys.join(";")})."
	     ...
	     # Connect to the remote batsd server over TCP
	        ...
	        def query_remote(command, attempt=0)
	            Timeout::timeout(self.timeout.to_f / 1000.0) do
	                ...
	                puts "Batsd: Timed out on #{command} #{"; retrying." if attempt < MAX_ATTEMPTS}"
	                if attempt < MAX_ATTEMPTS 
	                ...
	                if attempt < MAX_ATTEMPTS 
	                    ...
	                    puts "Batsd: Error querying remote server with #{command} due to #{e}: #{e.message} #{e.backtrace.join("\n")}"
	                    self.remote = nil
	                    raise e
	                end
	        end
